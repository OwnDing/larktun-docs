...
- **接入后服务器 DNS 异常**：部分云厂商（例如阿里云）的内网 DNS 地址落在 `100.64.0.0/10` 网段内，可能与云雀通地址段重叠。安装脚本已经附带 `/usr/lib/larktun/preserve-underlay-routes.sh` 来保留这类底层 DNS 路由；如果仍有问题，请按照[云雀通与阿里云：解决 Linux 与 Docker DNS 无法联网](../troubleshooting/larktun-alibaba-cloud-dns)排查。
