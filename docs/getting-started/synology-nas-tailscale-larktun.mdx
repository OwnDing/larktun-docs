...
description: 零基础图文教程：在群晖 Synology DSM 安装 Tailscale、接入云雀通，并让电脑通过 NAS 的 100.64.0.x 私有 IP 安全访问 DSM。
keywords:
  - 群晖 NAS 安装 Tailscale
  - 群晖 NAS 使用云雀通
...
1. 在电脑上安装并打开云雀通客户端。详细步骤参见[电脑客户端操作](./desktop-client-operations)。
2. 使用与群晖 NAS **相同的云雀通账号、租户或工作空间**登入。
...
:::important[不要照抄截图中的 IP]
截图里的 `100.64.0.2` 只是示例。请在云雀通设备列表中找到**你自己的群晖 NAS**，复制它对应的 `100.64.0.x` 地址。不要误用电脑本身的私有 IP。
:::

![电脑登入云雀通后使用群晖 NAS 的 100.64.0.x 私有 IP 打开 DSM 登录页面](/img/docs/quickstart/synology/synology-access-dsm-with-larktun-private-ip.webp)
...
5. 如果修改过 DSM 端口，请把地址中的 `5000` 或 `5001` 替换为实际端口。
...
- [电脑客户端操作](./desktop-client-operations)
